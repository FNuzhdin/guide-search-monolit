...
|---|---|---|---|
| `/api/search` | `GET` | `search`, `calculator`, `category`, `limit`, `page` | Поиск и фильтрация материалов. Возвращает `{ success, data, total, limit }`. `data` — массив до `limit` записей (по умолчанию 100, макс. 100, мин. 10, шаг 10). `total` — общее количество подходящих записей. `page` — смещение для отображения элементов постранично. |
...
- **Поле поиска** — ввод текста для поиска материалов. Поиск происходит автоматически с задержкой (debounce 300 мс) после окончания ввода. Ищет по полям `name`, `name_in_bitrix`, `category`, а также по числовому идентификатору `guide_id`:
  - если пользователь вводит целое число, поиск находит все материалы, в названии которых содержится это число, плюс материал с точным совпадением `guide_id`;
  - если вводится текст (содержит буквы или символы), поиск выполняется только по текстовым полям.
...
- **Счётчик результатов** — показывает общее количество найденных материалов.
...
- Первоначальная загрузка справочника может занять несколько секунд — в это время отображается крутящийся лоадер.
- При смене фильтров или поиска тоже отображается крутящийся лоадер.

### Обработка ошибок

- Если API возвращает ошибку или данные отсутствуют, отображается сообщение «Ничего не найдено».
- При проблемах с сетью или сервером пользователь видит сообщение с рекомендацией перезагрузить страницу и кнопку для повторной попытки.
