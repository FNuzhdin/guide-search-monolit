...
|---|---|---|---|
| `/api/search` | `GET` | `search`, `calculator`, `category`, `limit`, `page` | Поиск и фильтрация материалов. Возвращает `{ success, data, total, limit }`. `data` — массив до `limit` записей (по умолчанию 100, макс. 100, мин. 10, шаг 10). `total` — общее количество подходящих записей. `page` - смещение для отображение элементов постранично. |
...
- При смене фильтров или поиска появляется тоже отображается крутящийся лоадер.
